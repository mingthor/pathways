...
public:
    ...
    enum State {
        ...
        BLOCKED,    // The fiber is waiting for an event (e.g., mutex, condition variable, I/O)
                    // and has yielded control.
        TERMINATED  // The fiber has completed its execution.
    };

    // Constructor for a new fiber.
    // 'func': The function (now an absl::AnyInvocable) that this fiber will execute.
    // 'scheduler': A pointer to the global FiberScheduler instance, allowing the
    ...
    // Get a pointer to the jmp_buf structure, which stores the fiber's
    ...
    // The actual entry point for the fiber. This function is called when the fiber
    // is first switched to via `longjmp`. It sets up the thread-local `current_fiber`
    // and then executes the user-provided function.
    void RunEntryPoint();

private:
    int id_;                            // Unique identifier for this fiber.
    static std::atomic<int> next_id_;  // Static atomic counter for generating unique IDs.
