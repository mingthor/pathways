...
FiberScheduler::FiberScheduler(int num_threads)
    : shutdown_(false) {
    LOG(INFO) << "FiberScheduler initializing with " << num_threads << " threads.";
...
FiberScheduler::~FiberScheduler() {
    LOG(INFO) << "FiberScheduler shutting down.";
    ...
    LOG(INFO) << "FiberScheduler shutdown complete. Remaining fibers: " << fibers_.size();
    for (Fiber* f : fibers_) {
        delete f;
    }
}
...
void FiberScheduler::Schedule(Fiber* fiber) {
    ...
    LOG(INFO) << "Scheduled Fiber " << fiber->id() << ", state: READY. Ready queue size: " << ready_fibers_.size();
}
...
void FiberScheduler::Yield() {
    ...
    LOG(INFO) << "Fiber " << caller_fiber->id() << " yielding.";
    ...
    if (setjmp(*caller_fiber->context()) == 0) {
        ...
        LOG(INFO) << "Fiber " << caller_fiber->id() << " resumed.";
    }
}
...
void FiberScheduler::Block() {
    ...
    LOG(INFO) << "Fiber " << caller_fiber->id() << " blocking.";
...
void FiberScheduler::Unblock(Fiber* fiber) {
    ...
    LOG(INFO) << "Unblocking Fiber " << fiber->id() << ".";
    Schedule(fiber);
}

Fiber* FiberScheduler::GetCurrentFiber() const {
...
void FiberScheduler::WorkerThreadLoop() {
    ...
    if (setjmp(thread_context) == 0) {
        while (!shutdown_.load()) {
            ...
            if (fiber_to_run) {
                LOG(INFO) << "Worker thread " << std::this_thread::get_id()
                          << " picked Fiber " << fiber_to_run->id() << ". State: " << fiber_to_run->state();
                SwitchToFiber(fiber_to_run);
            }
        }
    } else {
        // This else block is where fibers longjmp back to when they yield/block/terminate.
        // The worker thread will then loop back to the `while` condition.
    }
    LOG(INFO) << "Worker thread " << std::this_thread::get_id() << " exiting.";
}
...
void FiberScheduler::SwitchToFiber(Fiber* fiber) {
    ...
    LOG(INFO) << "Switching to Fiber " << fiber->id() << ".";
...
void FiberScheduler::SwitchToSchedulerContext() {
    ...
    LOG(INFO) << "Switching back to scheduler context.";
